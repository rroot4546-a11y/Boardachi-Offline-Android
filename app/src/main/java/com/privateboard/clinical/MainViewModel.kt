...
class MainViewModel(app: Application) : AndroidViewModel(app) {
    ...
    fun requestAiBatch(questions:List<Question>){val missing=AiAnswerLogic.missingAnswers(questions);val settings=OpenRouterSettings(getApplication());if(!settings.hasToken()){aiMessage="Add an API key in Settings first.";return};if(missing.isEmpty()){aiMessage="No unanswered custom questions.";return};aiBatchJob=viewModelScope.launch{var done=0;try{for(q in missing){aiBatchProgress=done to missing.size;aiWorkingQuestionId=q.id;val answer=withContext(Dispatchers.IO){OpenRouterClient(settings).answer(q)};withContext(Dispatchers.IO){customRepo.saveAiAnswer(q,answer)};done++;aiBatchProgress=done to missing.size};aiMessage="$done AI-generated answers saved — verify medically.";loadCorpus()}catch(t:Throwable){if(t is kotlinx.coroutines.CancellationException)throw t;aiMessage="Batch stopped after $done: ${t.message}"}finally{aiBatchProgress=null;aiWorkingQuestionId=null;aiBatchJob=null}}}
    fun requestAiAnswer(question: Question) { val settings=OpenRouterSettings(getApplication());if(!settings.hasToken()){aiMessage="Add an API key in Settings first.";return};aiWorkingQuestionId=question.id;aiMessage=null;viewModelScope.launch{try{val answer=withContext(Dispatchers.IO){OpenRouterClient(settings).answer(question)};val updated=withContext(Dispatchers.IO){customRepo.saveAiAnswer(question,answer)};sessionQuestions=sessionQuestions.map{if(it.id==updated.id)updated else it};aiMessage="AI-generated answer saved — verify medically.";loadCorpus()}catch(t:Throwable){aiMessage=t.message?:"AI request failed."}finally{aiWorkingQuestionId=null}}}
